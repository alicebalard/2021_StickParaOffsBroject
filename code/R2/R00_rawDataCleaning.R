...
## For Alice's study: we need only G2 fish and their matching G1
table(cleanDF$clutch.ID, cleanDF$Generat)
# some fathers don't have G2: discard

table(cleanDF$clutch.ID, cleanDF$Generat) %>% unclass() %>% data.frame() %>% 
  dplyr::filter(O > 0) %>% row.names() -> clutch.ID2keep

# 132 fish
clean132 <- cleanDF[cleanDF$clutch.ID %in% clutch.ID2keep,] 

table(clean132$Generat) 
# 116 offspring, 16 parents

## Export cleaned dataset for reduced study (only G1 which have offspring)
write.csv(cleanDF[cleanDF$clutch.ID %in% clutch.ID2keep,], 
          "../../data/cleanedRawData132fishG1G2.csv", quote = F, row.names = F)
