...
RESPCA <- getPCACpG(DMSvec=unique(EffectsDF_ANNOT$DMS), effect="all effects")

## Save for later 
RESPCAgeneral <- RESPCA
# [1] "707 DMS linked with all effects"
# [1] "The chosen model is:"
# BCI ~ PCA1 + PAT + (1 | brotherPairID) + (1 | Sex) + PCA1:PAT
# [1] "535 CpG sites most correlated (p < 0.05) with the first principal component"
# [1] "336 CpG sites most correlated (p < 0.05) with the second principal component"
# PCA1:PAT                        0 23868.7 23868.7     1   107  7.3048 0.0080 **
  
## WEIRD: BCI do not correlate with nbr of worms??
ggplot(fullMetadata_OFFS, aes(x=No.Worms, y=BCI)) + geom_point() + geom_smooth(method = "lm")

objDMS = getG2methAtCpGs(EffectsDF_ANNOT$DMS)

ggplot(objDMS, aes(x=No.Worms, y=BCI)) + 
  geom_point() + geom_smooth(method = "lm")

## TBC here


## PREVIOUS W/O CORRECTION FOR GENETIC BACKGROUND
