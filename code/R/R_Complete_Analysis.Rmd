...
We perform a PCA on `r nrow(na.omit(percmeth))` CpG sites covered in all G2 individuals. We first perform a test on the complete dataset.
...
1. 
- raw beta values at DMS shared by >4 (or more) BP
or
...
We perform a PCA on `r nrow(na.omit(percAtDMS))` CpG sites covered in all G2 individuals. We first perform a test on the complete dataset.
...
- `r round(A, 2)`% of the variance in associated with the parasite load (number of worms)
- `r round(B, 2)`% of the variance in associated with the paternal treatment
- `r round(C, 2)`% of the variance in associated with the second PCA axis

### PCA based on all methylation values at DMS positions detected for parental effect, with imputation of missing values

We found `r nrow(percAtDMS)` DMS linked with parental effect (comparisons CC_TC and CT_TT)

We use missMDA and FactoMineR for imputation of missing data and performing of PCA:
(see:
