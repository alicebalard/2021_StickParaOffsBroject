...
```{r savePlotReacNorm, fig.height=10, fig.width=15}
plots$plot1
plots$plot2
plots$plot3
plots$plot4
...
pdf("../../dataOut/reactionNorms/caseVennG1only.pdf", width = 15, height = 10)
plots$plot1
dev.off()
pdf("../../dataOut/reactionNorms/caseVennG2only.pdf", width = 15, height = 10)
plots$plot2
dev.off()
pdf("../../dataOut/reactionNorms/caseVennG1G2NOinter.pdf", width = 15, height = 10)
plots$plot3
dev.off()
pdf("../../dataOut/reactionNorms/caseVennG1G2inter.pdf", width = 15, height = 10)
plots$plot4
dev.off()
pdf("../../dataOut/reactionNorms/caseVennG2interNOG1.pdf", width = 15, height = 10)
plots$plot5
dev.off()
```
...
plotAndsave <- function(i){
  ...
  print(P)
  pdf(paste0("../../dataOut/ManhattanPlots/Manhattan", namesCases[[i]], ".pdf"), width = 16, height = 5)
  P
}
...
pdf(GOplot, file = "../../dataOut/GOplot5Venncat.pdf", width = 10, height = 25)
GOplot
dev.off()
```
...
resPCACpG_G1G2inter <- getPCACpG(DMSvec=caseVennG1G2inter, effect="INTERACTION effect G1:G2")

# PCA plot of individuals with associated colors for paternal treatments
fviz_pca_ind(resPCACpG_G1G2inter$PCA_percAtDMS_imputed$res.PCA,  label="none", 
             habillage=as.factor(resPCACpG_G1G2inter$PCA_percAtDMS_imputed$metadata$PAT)) +
  scale_shape_manual(values=c(19,19))

# PCA plot of individuals with associated colors for brother pair
fviz_pca_ind(resPCACpG_G1G2inter$PCA_percAtDMS_imputed$res.PCA,  label="none",
             habillage=as.factor(resPCACpG_G1G2inter$PCA_percAtDMS_imputed$metadata$brotherPairID))

formula(resPCACpG_G1G2inter$PCA_percAtDMS_imputed$modSel)
...
mod_noworms = lmer(BCI ~ PCA2 + PAT + PCA2:PAT + (1 | brotherPairID) + (1 | Sex), 
                   data = resPCACpG_G1G2inter$PCA_percAtDMS_imputed$metadata)
...
# R2c conditional R2 value associated with fixed effects plus the random effects.
A = (MuMIn::r.squaredGLMM(resPCACpG_G1G2inter$PCA_percAtDMS_imputed$modSel)[2] -
       MuMIn::r.squaredGLMM(mod_noworms)[2])*100
B = (MuMIn::r.squaredGLMM(resPCACpG_G1G2inter$PCA_percAtDMS_imputed$modSel)[2] -
       MuMIn::r.squaredGLMM(mod_noPAT)[2])*100
C = (MuMIn::r.squaredGLMM(resPCACpG_G1G2inter$PCA_percAtDMS_imputed$modSel)[2] -
...
round(C, 2) #% of the variance in associated with the PCA axis

### Plot of the model
phenoMethPlot1 <- plot(ggpredict(resPCACpG_G1G2inter$PCA_percAtDMS_imputed$modSel, terms = c("No.Worms", "PCA2", "PAT")), add.data = TRUE, alpha = .08) + theme_bw() +
...
# save
pdf(file = "../../dataOut/phenotypeMeth/phenoMethPlot_G1:G2effect.pdf", width = 10, height = 5)
phenoMethPlot1
dev.off()

# Run model CpG by CpG, for the CpG in the second PCA axis
# myDF = data.frame()
# for (i in 1:nrow(percMethylation(resPCACpG_G1G2inter$CpGPCA2))){
#   print(i)
#   metadata = fullMetadata_OFFS
#   metadata$PCA2percMeth = percMethylation(resPCACpG_G1G2inter$CpGPCA2)[i,]
#   # rm NA in PCA
#   metadata=metadata[!is.na(metadata$PCA2percMeth),]
#   mod = lmer(BCI ~ PCA2percMeth*No.Worms*PAT + (1|brotherPairID)+ (1|Sex), data=metadata)
#   modNoMeth = lmer(BCI ~ No.Worms*PAT + (1|brotherPairID)+ (1|Sex), data=metadata)
#   print(lrtest(mod, modNoMeth))
#   myDF <- rbind(myDF, data.frame(CpG=paste(resPCACpG_G1G2inter$CpGPCA2[i]$chr, resPCACpG_G1G2inter$CpGPCA2[i]$end),
#                                  pvalue=lrtest(mod, modNoMeth)[["Pr(>Chisq)"]][2]))
# }

# FDR correction
# myDF[myDF$pvalue<0.05,]
# myDF[p.adjust(p = myDF$pvalue, method = "fdr")<0.05,] # No position
```
...
```{r BCI_PCA_G1}
# Make PCA and model lmer(BCI ~ PCA1*PCA2*No.Worms*PAT + (1|brotherPairID)+ (1|Sex), data=metadata)
resPCACpG_G1only <- getPCACpG(DMSvec=caseVennG1only, effect="G1 only")

formula(resPCACpG_G1only$PCA_percAtDMS_imputed$modSel)
# The SECOND PCA axis is significant in BCI
# BCI ~ PCA2 + No.Worms + PAT + (1 | brotherPairID) + (1 | Sex) + 
#   PCA2:PAT + No.Worms:PAT
...
### How much of the BCI variance is explained by each variables?
mod_noworms = lmer(BCI ~ PCA2 + PAT + PCA2:PAT + (1 | brotherPairID) + (1 | Sex), 
                   data = resPCACpG_G1only$PCA_percAtDMS_imputed$metadata)
mod_noPAT = lmer(BCI ~ PCA2 + No.Worms + (1 | brotherPairID) + (1 | Sex), 
                 data = resPCACpG_G1only$PCA_percAtDMS_imputed$metadata)
mod_noPCA2 = lmer(BCI ~ + No.Worms + PAT + No.Worms:PAT +(1 | brotherPairID) + (1 | Sex), 
                  data = resPCACpG_G1only$PCA_percAtDMS_imputed$metadata)
...
# R2c conditional R2 value associated with fixed effects plus the random effects.
A = (MuMIn::r.squaredGLMM(resPCACpG_G1only$PCA_percAtDMS_imputed$modSel)[2] -
       MuMIn::r.squaredGLMM(mod_noworms)[2])*100
B = (MuMIn::r.squaredGLMM(resPCACpG_G1only$PCA_percAtDMS_imputed$modSel)[2] -
       MuMIn::r.squaredGLMM(mod_noPAT)[2])*100
C = (MuMIn::r.squaredGLMM(resPCACpG_G1only$PCA_percAtDMS_imputed$modSel)[2] -
       MuMIn::r.squaredGLMM(mod_noPCA2)[2])*100
round(A, 2) #% of the variance in associated with the parasite load (number of worms)
round(B, 2) #% of the variance in associated with the paternal treatment
round(C, 2) #% of the variance in associated with the PCA axis
...
### Plot of the model
phenoMethPlot2 <- plot(ggpredict(resPCACpG_G1only$PCA_percAtDMS_imputed$modSel, terms = c("No.Worms", "PCA2", "PAT")), add.data = TRUE, alpha = .08) + theme_bw() +
  scale_color_gradient(low = "blue", high = "red")+
  scale_fill_gradient(low = "blue", high = "red") +
  ylab("Body Condition Index") + xlab("Number of worms")+
  ggtitle("Predicted values of Body Condition Index in offspring")
...
# save
pdf(file = "../../dataOut/phenotypeMeth/phenoMethPlot_G1onlyeffect.pdf", width = 10, height = 5)
phenoMethPlot2
dev.off()

# Run model CpG by CpG, for the CpG in the second PCA axis
# myDF = data.frame()
# for (i in 1:nrow(percMethylation(resPCACpG_G1only$CpGPCA2))){
#   print(i)
#   metadata = fullMetadata_OFFS
#   metadata$PCA2percMeth = percMethylation(resPCACpG_G1only$CpGPCA2)[i,]
#   # rm NA in PCA
#   metadata=metadata[!is.na(metadata$PCA2percMeth),]
#   mod = lmer(BCI ~ PCA2percMeth*No.Worms*PAT + (1|brotherPairID)+ (1|Sex), data=metadata)
#   modNoMeth = lmer(BCI ~ No.Worms*PAT + (1|brotherPairID)+ (1|Sex), data=metadata)
#   print(lrtest(mod, modNoMeth))
#   myDF <- rbind(myDF, data.frame(CpG=paste(resPCACpG_G1only$CpGPCA2[i]$chr, resPCACpG_G1only$CpGPCA2[i]$end),
#                                  pvalue=lrtest(mod, modNoMeth)[["Pr(>Chisq)"]][2]))
# }
# 
# # FDR correction
...
```{r BCI_PCA_G2}
# Make PCA and model lmer(BCI ~ PCA1*PCA2*No.Worms*PAT + (1|brotherPairID)+ (1|Sex), data=metadata)
resPCACpG_G2only <- getPCACpG(DMSvec=caseVennG2only, effect="G2 only")

formula(resPCACpG_G2only$PCA_percAtDMS_imputed$modSel)
# The FIRST PCA axis is significant in BCI
# BCI ~ PCA1 + No.Worms + PAT + (1 | brotherPairID) + (1 | Sex) + 
#   PCA1:PAT + No.Worms:PAT

### How much of the BCI variance is explained by each variables?
mod_noworms = lmer(BCI ~ PCA1 + PAT + PCA1:PAT + (1 | brotherPairID) + (1 | Sex), 
                   data = resPCACpG_G2only$PCA_percAtDMS_imputed$metadata)
mod_noPAT = lmer(BCI ~ PCA1 + No.Worms + (1 | brotherPairID) + (1 | Sex), 
                 data = resPCACpG_G2only$PCA_percAtDMS_imputed$metadata)
mod_noPCA1 = lmer(BCI ~ + No.Worms + PAT + No.Worms:PAT +(1 | brotherPairID) + (1 | Sex), 
                  data = resPCACpG_G2only$PCA_percAtDMS_imputed$metadata)

# R2c conditional R2 value associated with fixed effects plus the random effects.
A = (MuMIn::r.squaredGLMM(resPCACpG_G2only$PCA_percAtDMS_imputed$modSel)[2] -
       MuMIn::r.squaredGLMM(mod_noworms)[2])*100
B = (MuMIn::r.squaredGLMM(resPCACpG_G2only$PCA_percAtDMS_imputed$modSel)[2] -
       MuMIn::r.squaredGLMM(mod_noPAT)[2])*100
C = (MuMIn::r.squaredGLMM(resPCACpG_G2only$PCA_percAtDMS_imputed$modSel)[2] -
       MuMIn::r.squaredGLMM(mod_noPCA1)[2])*100
round(A, 2) #% of the variance in associated with the parasite load (number of worms)
round(B, 2) #% of the variance in associated with the paternal treatment
round(C, 2) #% of the variance in associated with the PCA axis

### Plot of the model
phenoMethPlot3 <- plot(ggpredict(resPCACpG_G2only$PCA_percAtDMS_imputed$modSel, terms = c("No.Worms", "PCA1", "PAT")), add.data = TRUE, alpha = .08) + theme_bw() +
  scale_color_gradient(low = "blue", high = "red")+
  scale_fill_gradient(low = "blue", high = "red") +
  ylab("Body Condition Index") + xlab("Number of worms")+
  ggtitle("Predicted values of Body Condition Index in offspring")
...
# save
pdf(file = "../../dataOut/phenotypeMeth/phenoMethPlot_G2onlyeffect.pdf", width = 10, height = 5)
phenoMethPlot3
dev.off()
```
