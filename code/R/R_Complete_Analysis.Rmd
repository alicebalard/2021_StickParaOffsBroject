...
# keep complete data: to few
# nrow(na.omit(percAtDMS))

# prcomp(percAtDMS, scale = TRUE, center = TRUE)
# BiocManager::install("pcaMethods")
# browseVignettes("pcaMethods")

## Other possibility: (computationally intensive) 
# http://juliejosse.com/wp-content/uploads/2018/05/DataAnalysisMissingR.html
# https://www.youtube.com/watch?v=OOM8_FH6_8o

# Perform PCA with missing values using the imputePCA functions, with the number of components determined by the estim_ncpPCA.
library(missMDA)
library("FactoMineR")
...
nb <- estim_ncpPCA(t(percAtDMS), scale = T) # estimate the number of components from incomplete data
nb$ncp # 5
res.comp <- imputePCA(t(percAtDMS), ncp = nb$ncp, scale = T) # impute the table
res.PCA <- PCA(res.comp$completeObs) # perform PCA

res.PCA <- PCA(t(head(percAtDMS))) # perform PCA

res.PCA$eig # take component 1 + 2 

# 3. extract axes 1 & 2
metadata = fullMetadata_OFFS

# check that the sample names are in the same order
ifelse(table(rownames(res.PCA$ind$coord) == fullMetadata_OFFS$SampleID), "sample names are in the same order", "ERROR in PCA sample names order")

metadata$PCA1 = res.PCA$ind$coord[,1] # axis 1
metadata$PCA2 = res.PCA$ind$coord[,2] # axis 2

# 4. Correlation with parasite load/BCI
mod_Tol.Meth3 <- lmer(BCI ~ PCA1*PCA2*No.Worms*PAT + (1|brotherPairID)+ (1|Sex), 
                     data=metadata, REML = F)

## Model selection:
step(mod_Tol.Meth3, reduce.random = F) 
# Model found: PCA2 axis has an effect

mod_Tol.Meth3_final <- lmer(BCI ~ PCA2 + No.Worms + PAT + (1 | brotherPairID) + (1 | Sex) + PCA2:PAT + No.Worms:PAT, 
                     data=metadata, REML = F)

mod_Tol.Meth3_final_NOPCA2 <- lmer(BCI ~ No.Worms + PAT + (1 | brotherPairID) + (1 | Sex) + No.Worms:PAT, 
                     data=metadata,  REML = F)

lrtest(mod_Tol.Meth3_final, mod_Tol.Meth3_final_NOPCA2)

## The slope of BCI on nbrworms varies upon treatment
plot(ggpredict(mod_Tol.Meth3_final, terms = c("No.Worms", "PCA2", "PAT")), add.data=T)+ theme_bw() #+
  # ylab("Body Condition Index") + xlab("Number of worms") +
  # ggtitle("Predicted values of Body Condition Index in offspring")+
  # scale_color_manual(NULL, values = c("black", "red")) +
  # scale_fill_manual(NULL, values = c("black", "red"))  +
