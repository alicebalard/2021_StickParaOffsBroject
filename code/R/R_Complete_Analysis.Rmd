...
```{r geneOnto_runGO}
# rm intersecting genes = genes with DMS belonging to several effects
temp=table(c(unique(DMS_G1onlyEffect_4BPmin_ANNOT$feature.name),
        unique(DMS_G2onlyEffect_4BPmin_ANNOT$feature.name),
        unique(DMS_G1G2additiveEffect_4BPmin_ANNOT$feature.name),
        unique(DMS_G1G2interactionEffect_4BPmin_ANNOT$feature.name)))
temp2rm=names(temp[temp>1])

GO_G1only = makedfGO(DMS_G1onlyEffect_4BPmin_ANNOT %>%
                       dplyr::filter(!feature.name %in% temp2rm)%>%
                       distinct(feature.name,.keep_all = TRUE), gene_universe, effect = "528 genes with G1 effect only")

GO_G2only = makedfGO(DMS_G2onlyEffect_4BPmin_ANNOT %>%
                       dplyr::filter(!feature.name %in% temp2rm)%>%
                       distinct(feature.name,.keep_all = TRUE) , gene_universe, effect = "101 genes with G2 effect only")

GO_G1G2addit = makedfGO(DMS_G1G2additiveEffect_4BPmin_ANNOT %>%
                          dplyr::filter(!feature.name %in% temp2rm)%>%
                          distinct(feature.name,.keep_all = TRUE), gene_universe, effect = "45 genes with additive effect only")

GO_G1G2inter = makedfGO(DMS_G1G2interactionEffect_4BPmin_ANNOT %>%
                          dplyr::filter(!feature.name %in% temp2rm)%>%
                          distinct(feature.name,.keep_all = TRUE), gene_universe, effect = "44 genes with interaction effect only")
...
pdf(GOplot, file = "../../dataOut/GOplot_fig3VennGenes_uniqueeffect.pdf", width = 6, height = 16)
GOplot
dev.off()
```
...
# 1639 DMS linked with G1
# [1] "The chosen model is:"
# BCI ~ PCA2 + No.Worms + PAT + (1 | brotherPairID) + (1 | Sex) + 
#     PCA2:PAT + No.Worms:PAT
# PCA2:PAT                        0  39444   39444     1   105 12.4623 0.000618 ***
# No.Worms:PAT                    0  20484   20484     1   105  6.4719 0.012415 *  
# [1] "860 CpG sites most correlated (p < 0.05) with the first principal component"
# [1] "827 CpG sites most correlated (p < 0.05) with the second principal component"
...
# BCI ~ PCA2 + No.Worms + PAT + (1 | brotherPairID) + (1 | Sex) + 
#     PCA2:PAT + No.Worms:PAT
# PCA2:PAT                        0 28518.0 28518.0     1 105.000  8.9750 0.003415 **
# No.Worms:PAT                    0 26923.2 26923.2     1 105.000  8.4731 0.004402 **
# [1] "76 CpG sites most correlated (p < 0.05) with the first principal component"
...
### Plot annotated Manhattan plots for those 435 genes associated with PCA2!
...
```{r geneOnto_runGO_435genes_axis2}
...
```{r geneOnto_runGO_435genes_axis2_SPLITBYEFFECT}
